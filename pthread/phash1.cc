...
HashMap::HashMap() {
  rwEntrylock = new RWLock[TABLE_SIZE];
...
int HashMap:: get(int key) {
  ...
  rwEntrylock[hash].startRead();
  if (table[hash] == NULL){
    rwEntrylock[hash].doneRead(); //(2) done reading
    return -1;
  }
  else {
...
void HashMap::put(int key, int value) {
  ...
  rwEntrylock[hash].doneWrite(); //(2) done writing
}
